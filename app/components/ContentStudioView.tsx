...
export default function ContentStudioView({ sidebarJSX }) {
  ...
  const isRandomPick = selectedPromptId === RANDOM_PICK;
  // معاينة ثابتة فقط للعرض — السحب العشوائي الفعلي يحدث عند الضغط على توليد
  const randomPreviewRef = useRef<any>(null);
  if (isRandomPick) {
    if (!randomPreviewRef.current || !promptsForSelection.find(p => p.id === randomPreviewRef.current?.id)) {
      randomPreviewRef.current = promptsForSelection.length > 0 ? promptsForSelection[Math.floor(Math.random() * promptsForSelection.length)] : null;
    }
  } else {
    randomPreviewRef.current = null;
  }
  const selectedPrompt = isRandomPick ? randomPreviewRef.current : loadLib().find(p => p.id === selectedPromptId);
  ...
  if (prevCatType.current.category !== category || prevCatType.current.activeType !== activeType) {
    ...
    else if (np.length === 1) setSelectedPromptId(np[0].id);
    else setSelectedPromptId("");
  }

  // سحب prompt عشوائي جديد — يُستخدم عند كل عملية توليد فعلية
  const pickPromptForGeneration = () => {
    if (!isRandomPick) return selectedPrompt;
    const pool = loadLib().filter(p => p.category === category && p.type === activeType);
  ...
  const generateWithGemini = async () => {
    ...
    try {
      ...
      setHistory(h => [{ id:`gen_${Date.now()}`, promptName: activePrompt.name, category, type: activeType, typeLabel: TYPE_LABELS[activeType], imageUrl: data.image_url, time: iteration.time }, ...h.slice(0,19)]);
          ...
          <div style={{ background:"var(--color-background-secondary)", borderLeft:"0.5px solid var(--color-border-tertiary)", padding:"16px", overflowY:"auto", display:"flex", flexDirection:"column", gap:"12px" }}>
            ...
            <div>
              <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:"4px" }}>
                ...
                {promptsForSelection.length > 1 && (
                  <span style={{ fontSize:"9px", color:"var(--color-text-tertiary)" }}>{promptsForSelection.length} أوامر متاحة</span>
                )}
              </div>
              {promptsForSelection.length > 0 ? (
                <select value={selectedPromptId} onChange={e => setSelectedPromptId(e.target.value)} style={iStyle}>
                  {promptsForSelection.length > 1 && (
                    <option value={RANDOM_PICK}>🎲 تلقائي (عشوائي بين {promptsForSelection.length} أوامر)</option>
                  )}
              ...
              {isRandomPick && selectedPrompt && (
                <p style={{ fontSize:"10px", color:"#534AB7", margin:"4px 0 0" }}>🎲 سيتم اختيار أمر مختلف عشوائياً مع كل توليد — معاينة حالية: {selectedPrompt.name}</p>
              )}
            </div>
