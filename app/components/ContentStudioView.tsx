...
const REFINEMENT_PRESETS = [
  { label:"💡 إضاءة أقوى",   prompt:"increase the lighting brightness, make it more vibrant and well-lit" },
  { label:"🌿 أضف نباتات",   prompt:"add subtle green plants in the background for a natural feel" },
  { label:"☀️ نهاري",        prompt:"change to bright natural daylight scene, warm sunlight from windows" },
  { label:"🌙 ليلي",         prompt:"change to cozy evening/night scene, warm ambient lighting" },
  { label:"🪵 أرضية خشب",   prompt:"change the floor to light oak wood flooring" },
  { label:"🏔️ خلفية بيضاء", prompt:"change background to pure white seamless studio background" },
  { label:"🎨 ألوان أعمق",   prompt:"enhance colors to be richer and deeper, more saturated" },
  { label:"✨ تحسين جودة",   prompt:"enhance overall image quality, sharper details, more photorealistic" },
];
...
export default function ContentStudioView({ sidebarJSX }) {
  ...
  const [refineTarget, setRefineTarget] = useState(null); // {type, imageUrl}
  ...
  const refineImage = async (type, currentImageUrl) => {
    if (!refinePrompt.trim()) return alert("اكتب أمر التحسين أولاً");
    setRefineLoading(true);
    setRefineTarget({ type, imageUrl: currentImageUrl });
    try {
      const res = await fetch(`${API_URL}/content-studio/generate-image`, {
        ...
        body: JSON.stringify({
          ...
          mode: type, width:1024, height:1024
        })
      });
      if (!res.ok) { const err=await res.json(); throw new Error(err.detail||"خطأ"); }
      const data = await res.json();
      setGenImages(p => ({ ...p, [type]: data.image_url }));
      setHistory(h => [{ id:`refine_${type}_${Date.now()}`, promptName:`تحسين: ${refinePrompt.substring(0,30)}...`, category, type, typeLabel:TYPE_LABELS[type], imageUrl:data.image_url, time:new Date().toLocaleTimeString("ar") }, ...h.slice(0,19)]);
      setRefinePrompt("");
