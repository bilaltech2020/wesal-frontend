...
export default function Home() {
    ...
    <div style={{ width: "240px", background: "#f0f0f7", borderLeft: "1px solid #d8d8ee", padding: "32px 20px", display: "flex", flexDirection: "column", gap: "8px", minHeight: "100vh", flexShrink: 0 }}>
      ...
      {NAV.map(item => (
        <div key={item.label} onClick={() => setView(item.v as ViewType)}
          style={{ padding: "10px 14px", borderRadius: "10px", cursor: "pointer", display: "flex", alignItems: "center", gap: "10px", background: view === item.v && (item.v === "competitors" || item.v === "inventory") ? "#e4e4f4" : view === "dashboard" && item.v === "dashboard" && item.label === "لوحة التحكم" ? "#e4e4f4" : "transparent", color: (view === item.v && (item.v === "competitors" || item.v === "inventory")) || (view === "dashboard" && item.label === "لوحة التحكم") ? "#7c3aed" : "#666", fontSize: "14px", fontWeight: "500" }}>
          <span>{item.icon}</span>{item.label}
        </div>
      ))}
      <div style={{ marginTop: "auto", padding: "10px 14px", borderRadius: "10px", background: "#e4e4f4", cursor: "pointer" }} onClick={() => { localStorage.removeItem("wesal_token"); localStorage.removeItem("wesal_user"); setView("landing"); setUser(null); }}>
    ...
    <div style={{ fontFamily: "'Tajawal', sans-serif", direction: "rtl", minHeight: "100vh", background: "#ffffff", color: "#1a1a2e" }}>
      ...
      <div style={{ display: "flex", minHeight: "100vh" }}>
        ...
        <div style={{ flex: 1, padding: "40px", overflowY: "auto" }}>
          ...
          <p style={{ color: "#666", fontSize: "13px", margin: "0 0 28px" }}>ارفع ملف Excel أو ابحث يدوياً في عدة متاجر بنفس الوقت</p>
            ...
            <div>
              ...
              {stores.length > 0 && (
                <div style={{ background: "#f0f0f7", border: "1px solid #d8d8ee", borderRadius: "16px", overflow: "hidden", marginBottom: "12px" }}>
                  <div style={{ padding: "12px 16px", borderBottom: "1px solid #d8d8ee", fontSize: "11px", color: "#666", fontWeight: "600" }}>المتاجر ({stores.length})</div>
                  {stores.map(s => (
                    <div key={s.id} style={{ display: "flex", alignItems: "center", padding: "12px 16px", borderBottom: "1px solid #e0e0f0", gap: "8px" }}>
                      ...
                      <div style={{ flex: 1, overflow: "hidden" }}>
                        ...
                        <div style={{ fontSize: "11px", color: "#666", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", direction: "ltr", textAlign: "left" }}>{s.urlTemplate || s.url}</div>
                        {s.urlTemplate && <div style={{ fontSize: "10px", color: "#4ade80", marginTop: "2px" }}>✓ template رابط</div>}
                      </div>
                      <button onClick={() => setStores(p => p.filter(x => x.id !== s.id))} style={{ background: "none", border: "none", color: "#666", cursor: "pointer", fontSize: "13px" }}>✕</button>
                    </div>
                  ))}
                </div>
              )}
                ...
                {!excelRows.length ? (
                  <div>
                    ...
                    >
                      ...
                      <div style={{ fontSize: "12px", color: "#666" }}>.xlsx أو .xls</div>
                    </div>
                  ...
                  <div>
                    <div style={{ background: "#f2f2fa", border: "1px solid #d8d8ee", borderRadius: "8px", padding: "10px 14px", display: "flex", alignItems: "center", gap: "10px", marginBottom: "14px" }}>
                      ...
                      <span style={{ marginRight: "auto", fontSize: "12px", color: "#666" }}>{excelRows.length} صف</span>
                      <button onClick={() => { setExcelRows([]); setExcelCols([]); setExcelResults([]); setExcelFileName(""); }} style={{ background: "none", border: "none", color: "#666", cursor: "pointer", fontSize: "13px" }}>✕</button>
                    </div>
                    ...
                    <div style={{ background: "#ffffff", border: "1px solid #d8d8ee", borderRadius: "8px", overflow: "hidden", marginBottom: "14px" }}>
                      <table style={{ width: "100%", borderCollapse: "collapse", fontSize: "12px" }}>
                        <thead>
                          <tr>{excelCols.slice(0, 4).map(c => <th key={c} style={{ padding: "8px 12px", textAlign: "right", color: "#666", borderBottom: "1px solid #d8d8ee", fontWeight: "500" }}>{c}</th>)}</tr>
                        </thead>
                        <tbody>
                          ...
                          {excelRows.length > 3 && <tr><td colSpan={4} style={{ padding: "7px 12px", color: "#666", fontStyle: "italic", fontSize: "11px" }}>... و {excelRows.length - 3} صف آخر</td></tr>}
                        </tbody>
                      </table>
                    </div>
                    ...
                    <button
                      ...
                      style={{ width: "100%", padding: "12px", background: excelSearching || stores.length === 0 ? "#c8c8e8" : "#7c3aed", color: excelSearching || stores.length === 0 ? "#666" : "#ffffff", border: "none", borderRadius: "10px", fontSize: "14px", fontWeight: "700", cursor: excelSearching || stores.length === 0 ? "not-allowed" : "pointer", fontFamily: "inherit" }}
                    >
              ...
              <div style={{ background: "#f0f0f7", border: "1px solid #d8d8ee", borderRadius: "16px", padding: "20px", marginBottom: "20px" }}>
                ...
                <div style={{ display: "flex", gap: "10px" }}>
                  ...
                  <button onClick={doSearch} disabled={searching || !skuInput.trim() || stores.length === 0}
                    style={{ padding: "12px 22px", background: (searching || stores.length === 0) ? "#c8c8e8" : "#7c3aed", color: (searching || stores.length === 0) ? "#666" : "#ffffff", border: "none", borderRadius: "10px", fontSize: "14px", fontWeight: "700", cursor: (searching || stores.length === 0) ? "not-allowed" : "pointer", fontFamily: "inherit", whiteSpace: "nowrap" }}>
                    {searching ? "جاري..." : "بحث ←"}
                  </button>
                </div>
              </div>
                  ...
                  <div style={{ padding: "14px 20px", borderBottom: "1px solid #d8d8ee", display: "flex", alignItems: "center", gap: "12px" }}>
                    ...
                    {!excelSearching && (
                      <button
                        ...
                        style={{ marginRight: "auto", padding: "6px 14px", background: "#e4e4f4", border: "1px solid #b8b8e0", borderRadius: "8px", color: "#7c3aed", fontSize: "12px", cursor: "pointer", fontFamily: "inherit" }}
                      >
                        تصدير Excel ↓
                      </button>
                    )}
                    ...
                    <table style={{ width: "100%", borderCollapse: "collapse", fontSize: "13px", minWidth: "600px" }}>
                      <thead>
                        <tr style={{ background: "#ffffff" }}>
                          <th style={{ padding: "9px 14px", textAlign: "right", color: "#666", fontWeight: "500", borderBottom: "1px solid #d8d8ee", width: "30px" }}>#</th>
                          <th style={{ padding: "9px 14px", textAlign: "right", color: "#666", fontWeight: "500", borderBottom: "1px solid #d8d8ee" }}>اسم المنتج</th>
                          <th style={{ padding: "9px 14px", textAlign: "right", color: "#666", fontWeight: "500", borderBottom: "1px solid #d8d8ee", width: "90px" }}>SKU</th>
                          <th style={{ padding: "9px 14px", textAlign: "right", color: "#666", fontWeight: "500", borderBottom: "1px solid #d8d8ee", width: "100px" }}>المتجر</th>
                          <th style={{ padding: "9px 14px", textAlign: "center", color: "#666", fontWeight: "500", borderBottom: "1px solid #d8d8ee", width: "100px" }}>السعر</th>
                          <th style={{ padding: "9px 14px", textAlign: "center", color: "#666", fontWeight: "500", borderBottom: "1px solid #d8d8ee", width: "90px" }}>الحالة</th>
                          <th style={{ padding: "9px 14px", textAlign: "center", color: "#666", fontWeight: "500", borderBottom: "1px solid #d8d8ee", width: "60px" }}>رابط</th>
                        </tr>
                      </thead>
                      <tbody>
                        {excelResults.map((r, i) => (
                          <tr key={i} style={{ borderBottom: "1px solid #e0e0f0", background: i % 2 === 0 ? "transparent" : "#f2f2fa" }}>
                            ...
                            <td style={{ padding: "10px 14px", textAlign: "center" }}>
                              {r.url ? <a href={r.url} target="_blank" rel="noopener noreferrer" style={{ color: "#7c6af7", fontSize: "12px", textDecoration: "none" }}>فتح ↗</a> : <span style={{ color: "#aaaacc" }}>—</span>}
                            </td>
                          </tr>
                        ))}
                      </tbody>
              ...
              {searches.length > 0 && searches.map(search => (
                <div key={search.id} style={{ background: "#f0f0f7", border: "1px solid #d8d8ee", borderRadius: "16px", overflow: "hidden", marginBottom: "20px" }}>
                  <div style={{ padding: "14px 20px", borderBottom: "1px solid #d8d8ee", display: "flex", alignItems: "center", gap: "12px" }}>
                    ...
                    <span style={{ fontSize: "11px", color: "#666" }}>{search.searchedAt}</span>
                  ...
                  {search.results.map(r => (
                    <div key={r.storeId}>
                      <div style={{ padding: "10px 20px", background: "#0f0f1a", display: "flex", alignItems: "center", gap: "8px", borderBottom: "1px solid #d8d8ee" }}>
                        ...
                        {r.status === "loading" && <span style={{ display: "flex", alignItems: "center", gap: "6px", fontSize: "12px", color: "#666" }}><span style={{ width: 10, height: 10, border: "2px solid #333", borderTopColor: "#7c3aed", borderRadius: "50%", display: "inline-block", animation: "spin 0.8s linear infinite" }} />جاري البحث...</span>}
                        {r.status === "done" && <span style={{ fontSize: "12px", color: "#666" }}>{r.count} منتج</span>}
                      ...
                      {r.status === "done" && r.products.length > 0 && (
                        <table style={{ width: "100%", borderCollapse: "collapse", fontSize: "13px" }}>
                          <thead><tr style={{ background: "#ffffff" }}>
                            <th style={{ padding: "8px 16px", textAlign: "right", color: "#666", fontWeight: "500", borderBottom: "1px solid #d8d8ee" }}>اسم المنتج</th>
                            <th style={{ padding: "8px 16px", textAlign: "center", color: "#666", fontWeight: "500", borderBottom: "1px solid #d8d8ee", width: "110px" }}>السعر</th>
                            <th style={{ padding: "8px 16px", textAlign: "center", color: "#666", fontWeight: "500", borderBottom: "1px solid #d8d8ee", width: "70px" }}>رابط</th>
                          </tr></thead>
                          <tbody>
                      ...
                      {r.status === "done" && r.products.length === 0 && <div style={{ padding: "16px 20px", fontSize: "13px", color: "#666" }}>لا توجد نتائج</div>}
                    </div>
                  ))}
                </div>
              ))}
  ...
  if (view === "reports") {
    ...
    const sb = (s) => s === "critical" ? "#fdeaea" : s === "warning" ? "#1a1400" : "#0d1f0d";
        ...
        <div style={{background:"#f2f2fa",border:"1px solid #378ADD",borderRadius:"14px",padding:"18px",marginBottom:"10px",animation:"fadeIn .2s ease"}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:"14px"}}>
            <div>
              ...
              <div style={{fontSize:"11px",color:"#666",marginTop:"2px"}}>{k.val} {k.unit} · ERPNext</div>
            </div>
            <button onClick={()=>setActiveKpi(null)} style={{padding:"4px 12px",background:"#e4e4f4",border:"1px solid #b8b8e0",borderRadius:"6px",color:"#666",fontSize:"12px",cursor:"pointer",fontFamily:"inherit"}}>✕ إغلاق</button>
          </div>
            ...
            <div style={{background:"#ffffff",borderRadius:"10px",overflow:"hidden",marginBottom:"12px"}}>
              <table style={{width:"100%",borderCollapse:"collapse",fontSize:"12px"}}>
                <thead><tr style={{background:"#f0f0f7"}}>
                  {k.key==="late_orders" && ["الطلب","العميل","أيام التأخير","المبلغ"].map(h=><th key={h} style={{padding:"8px 10px",textAlign:"right",color:"#666",fontWeight:"600",borderBottom:"1px solid #d8d8ee"}}>{h}</th>)}
                  {k.key==="stuck_orders" && ["الطلب","العميل","الحالة","التاريخ"].map(h=><th key={h} style={{padding:"8px 10px",textAlign:"right",color:"#666",fontWeight:"600",borderBottom:"1px solid #d8d8ee"}}>{h}</th>)}
                  {["out_of_stock","low_stock"].includes(k.key) && ["SKU","المستودع","الكمية"].map(h=><th key={h} style={{padding:"8px 10px",textAlign:"right",color:"#666",fontWeight:"600",borderBottom:"1px solid #d8d8ee"}}>{h}</th>)}
                  {k.key==="late_po" && ["PO","المورد","الاستحقاق","الحالة"].map(h=><th key={h} style={{padding:"8px 10px",textAlign:"right",color:"#666",fontWeight:"600",borderBottom:"1px solid #d8d8ee"}}>{h}</th>)}
                  {k.key==="open_complaints" && ["#","العميل","الموضوع","الحالة"].map(h=><th key={h} style={{padding:"8px 10px",textAlign:"right",color:"#666",fontWeight:"600",borderBottom:"1px solid #d8d8ee"}}>{h}</th>)}
                </tr></thead>
                <tbody>
                  {items.slice(0,10).map((item,i)=>(
                    <tr key={i} style={{borderBottom:"1px solid #e0e0f0",background:i%2===0?"transparent":"#f2f2fa"}}>
                      {k.key==="late_orders" && <>
                        ...
                        <td style={{padding:"7px 10px"}}><span style={{background:item.days_late>30?"#fdeaea":"#1a1400",color:item.days_late>30?"#f87171":"#fbbf24",padding:"2px 8px",borderRadius:"10px",fontSize:"10px"}}>{item.days_late} يوم</span></td>
                      ...
                      {k.key==="late_po" && <>
                        ...
                        <td style={{padding:"7px 10px"}}><span style={{background:"#fdeaea",color:"#f87171",padding:"2px 8px",borderRadius:"10px",fontSize:"10px"}}>{item.status}</span></td>
                      </>}
              ...
              {items.length > 10 && <div style={{padding:"8px",fontSize:"11px",color:"#666",textAlign:"center"}}>+ {items.length-10} سجل إضافي</div>}
            </div>
          )}
          <div style={{display:"flex",gap:"8px",flexWrap:"wrap"}}>
            ...
            <button style={{padding:"7px 14px",background:"#e4e4f4",border:"1px solid #c8b8ff",borderRadius:"8px",color:"#7c3aed",fontSize:"12px",cursor:"pointer",fontFamily:"inherit"}}>📊 Excel</button>
            <a href="http://144.91.102.29" target="_blank" rel="noopener noreferrer" style={{padding:"7px 14px",background:"#e4e4f4",border:"1px solid #555",borderRadius:"8px",color:"#666",fontSize:"12px",cursor:"pointer",fontFamily:"inherit",textDecoration:"none"}}>🔗 ERPNext ↗</a>
          </div>
        </div>
      );
    };
    ...
    const renderKCard = (k, i) => (
      <div onClick={()=>setActiveKpi(activeKpi===i?null:i)}
        style={{background:"#f0f0f7",border:`1px solid ${activeKpi===i?"#378ADD":"#2a2a4e"}`,borderTop:`3px solid ${sc(k.sev)}`,borderRadius:"12px",padding:"14px",cursor:"pointer",position:"relative",transition:"all .15s"}}>
        ...
        <div style={{fontSize:"24px",fontWeight:"700",color:sc(k.sev),lineHeight:"1.1",marginBottom:"2px"}}>
          ...
          <span style={{fontSize:"12px",fontWeight:"400",color:"#666",marginRight:"4px"}}>{k.unit}</span>
        </div>
        <div style={{fontSize:"10px",color:"#666"}}>{k.target}</div>
        <div style={{height:"3px",background:"#e4e4f4",borderRadius:"2px",marginTop:"8px"}}>
            ...
            <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:"20px"}}>
              <div>
                ...
                <div style={{fontSize:"12px",display:"flex",alignItems:"center",gap:"6px"}}>
                  {erpLoading?<span style={{color:"#666",display:"flex",alignItems:"center",gap:"4px"}}><span style={{width:7,height:7,border:"2px solid #333",borderTopColor:"#7c3aed",borderRadius:"50%",display:"inline-block",animation:"spin .8s linear infinite"}}/>جاري الجلب...</span>
              ...
              <div style={{display:"flex",gap:"6px",alignItems:"center"}}>
                {["اليوم","الأسبوع","الشهر","الربع","السنة"].map((p,i)=>(
                  <button key={p} onClick={()=>{setTimePeriod(i);fetchKpis(i);}} style={{padding:"5px 12px",background:timePeriod===i?"#e4e4f4":"transparent",border:`1px solid ${timePeriod===i?"#7c3aed":"#c8c8e8"}`,borderRadius:"20px",color:timePeriod===i?"#7c3aed":"#666",fontSize:"12px",cursor:"pointer",fontFamily:"inherit"}}>{p}</button>
                ))}
                <button onClick={()=>fetchKpis()} disabled={erpLoading} style={{padding:"5px 14px",background:"#e4e4f4",border:"1px solid #c8b8ff",borderRadius:"20px",color:"#7c3aed",fontSize:"12px",cursor:"pointer",fontFamily:"inherit",marginRight:"4px"}}>↻ تحديث</button>
              </div>
            </div>

            <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:"10px",marginBottom:"18px"}}>
              {[{l:"🔥 حرج",c:critical,col:"#E24B4A",bg:"#fdeaea",br:"#f8d0d0"},{l:"⚠️ تحذير",c:warning,col:"#EF9F27",bg:"#1a1400",br:"#3a2800"},{l:"✅ جيد",c:good,col:"#1D9E75",bg:"#0d1f0d",br:"#d0f0d0"}].map(g=>(
                <div key={g.l} style={{background:g.bg,border:`1px solid ${g.br}`,borderRadius:"12px",padding:"14px 16px"}}>
                  ...
                  <div style={{fontSize:"10px",color:"#666",marginTop:"2px"}}>مؤشر</div>
                </div>
              ))}
            </div>
            ...
            <div style={{display:"grid",gridTemplateColumns:"1.2fr .8fr",gap:"12px",marginTop:"8px"}}>
              <div style={{background:"#f0f0f7",border:"1px solid #d8d8ee",borderRadius:"14px",padding:"18px"}}>
                ...
                <div style={{display:"flex",gap:"8px"}}>
                  ...
                  <button style={{padding:"7px 14px",background:"#e4e4f4",border:"1px solid #c8b8ff",borderRadius:"8px",color:"#7c3aed",fontSize:"12px",cursor:"pointer",fontFamily:"inherit"}}>💬 واتساب للإدارة</button>
                </div>
              </div>
              <div style={{background:"#f0f0f7",border:"1px solid #d8d8ee",borderRadius:"14px",padding:"18px"}}>
                ...
                <a href="http://144.91.102.29" target="_blank" rel="noopener noreferrer" style={{display:"block",padding:"8px",background:"#e4e4f4",border:"1px solid #b8b8e0",borderRadius:"8px",color:"#7c3aed",fontSize:"12px",textDecoration:"none",textAlign:"center"}}>🔗 فتح ERPNext ↗</a>
              </div>
            </div>
    ...
    <div style={{ fontFamily:"'Tajawal',sans-serif", direction:"rtl", minHeight:"100vh", background:"#ffffff", color:"#1a1a2e" }}>
      ...
      <div style={{ display:"flex", minHeight:"100vh" }}>
        ...
        <div style={{ flex:1, padding:"40px", overflowY:"auto" }}>
          ...
          <p style={{ color:"#666", fontSize:"13px", margin:"0 0 24px" }}>أدخل المنتج والموقع في كل صف — ابحث في أي موقع تريده</p>
          ...
          <div style={{ display:"flex", borderBottom:"1px solid #d8d8ee", marginBottom:"20px" }}>
            {[
              ...
              <button key={t.k} onClick={() => setCompTab(t.k as any)}
                style={{ padding:"9px 20px", background:"none", border:"none", borderBottom:compTab===t.k?"2px solid #c8b8ff":"2px solid transparent", color:compTab===t.k?"#7c3aed":"#666", fontSize:"14px", fontWeight:compTab===t.k?"700":"400", cursor:"pointer", fontFamily:"inherit", marginBottom:"-1px" }}>
                {t.l}
              </button>
            ))}
          </div>
          ...
          {compTab === "manual" && (
            <div>
              {/* Table header */}
              <div style={{ display:"grid", gridTemplateColumns:"140px 1fr 180px 32px", gap:"8px", padding:"0 4px 8px", fontSize:"12px", color:"#666", fontWeight:"600" }}>
                <span>SKU</span>
                ...
                {compRows.map((row, idx) => (
                  <div key={row.id} style={{ display:"grid", gridTemplateColumns:"140px 1fr 180px 32px", gap:"8px", background:"#f0f0f7", border:"1px solid #d8d8ee", borderRadius:"10px", padding:"9px 12px", alignItems:"center" }}>
                    ...
                    <button onClick={() => compRemoveRow(row.id)} disabled={compRows.length===1}
                      style={{ width:30, height:30, background:"none", border:"1px solid #c0c0e0", borderRadius:"6px", color:"#666", cursor:compRows.length===1?"not-allowed":"pointer", fontSize:"13px", opacity:compRows.length===1?0.3:1 }}>✕</button>
                  </div>
                ))}
              </div>
              ...
              <div style={{ display:"flex", gap:"8px", marginBottom:"24px", flexWrap:"wrap" }}>
                ...
                <button onClick={compSearchAll} disabled={compSearching || !compRows.some(r => r.query.trim())}
                  style={{ padding:"9px 22px", background:compSearching||!compRows.some(r=>r.query.trim())?"#c8c8e8":"#7c3aed", color:compSearching||!compRows.some(r=>r.query.trim())?"#666":"#ffffff", border:"none", borderRadius:"10px", fontSize:"14px", fontWeight:"700", cursor:compSearching||!compRows.some(r=>r.query.trim())?"not-allowed":"pointer", fontFamily:"inherit" }}>
              ...
              {compRows.filter(r=>r.status!=="idle").map(row => (
                <div key={row.id} style={{ background:"#f0f0f7", border:"1px solid #d8d8ee", borderRadius:"14px", marginBottom:"10px", overflow:"hidden" }}>
                  <div style={{ padding:"11px 16px", borderBottom:"1px solid #d8d8ee", display:"flex", alignItems:"center", gap:"10px" }}>
                    ...
                    {row.sku && <span style={{ fontSize:"10px", color:"#666", fontFamily:"monospace", background:"#ffffff", padding:"2px 5px", borderRadius:"3px" }}>{row.sku}</span>}
                    {(row as any).site && <span style={{ fontSize:"11px", color:"#666", direction:"ltr" }}>{(row as any).site}</span>}
                    <span style={{ marginRight:"auto", fontSize:"11px", color:"#666" }}>
                    ...
                    <button onClick={()=>compSearchRow(row.id)} disabled={row.status==="searching"}
                      style={{ padding:"3px 9px", background:"none", border:"1px solid #c0c0e0", borderRadius:"5px", color:"#666", cursor:"pointer", fontSize:"11px", fontFamily:"inherit" }}>↻</button>
                    {row.status==="done" && row.results.filter(r=>r.title).length > 0 && (
                      <button onClick={()=>compAnalyzeResults(row.id)} disabled={compAnalyzing}
                        style={{ padding:"3px 10px", background:compAnalyzing?"#c8c8e8":"#1a2a1a", border:"1px solid #2a4a2a", borderRadius:"5px", color:compAnalyzing?"#666":"#4ade80", cursor:compAnalyzing?"not-allowed":"pointer", fontSize:"11px", fontFamily:"inherit" }}>
                  ...
                  {row.status==="done" && row.results.map((r,i) => {
                    ...
                    <div key={i} style={{ borderBottom:"1px solid #e0e0f0", background:i%2?"#f2f2fa":"transparent" }}>
                      <div style={{ display:"grid", gridTemplateColumns:"110px 1fr 120px 90px 65px", alignItems:"center", padding:"9px 16px", gap:"10px" }}>
                        <span style={{ fontSize:"11px", padding:"3px 8px", background:"#e4e4f4", color:"#7c3aed", borderRadius:"6px", textAlign:"center", fontWeight:"600", overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap" }}>{r.competitor}</span>
                        <div>
                          <div style={{ fontSize:"12px", overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap", color:r.title?"#1a1a2e":"#666" }}>{r.title||r.error||"—"}</div>
                          {ai && <div style={{ fontSize:"10px", color:"#666", marginTop:"2px" }}>
                            <span style={{ color: ai.confidence >= 75 ? "#4ade80" : ai.confidence >= 60 ? "#ffd166" : "#f87171" }}>{ai.match_type}</span>
                            <span style={{ margin:"0 4px", color:"#aaaacc" }}>·</span>
                            <span>{ai.match_reason}</span>
                          </div>}
                        </div>
                        <span style={{ fontSize:"13px", fontWeight:"700", color:r.price?"#7c3aed":"#666" }}>{r.price?`${r.price.toLocaleString()} ${r.currency}`:"—"}</span>
                        <span style={{ display:"inline-flex", alignItems:"center", gap:"4px", padding:"2px 8px", borderRadius:"20px", fontSize:"11px", background:r.available===true?"#0d1f0d":r.available===false?"#fdeaea":"#e4e4f4", color:r.available===true?"#4ade80":r.available===false?"#f87171":"#666", border:`1px solid ${r.available===true?"#d0f0d0":r.available===false?"#f8d0d0":"#c8c8e8"}` }}>
                          <span style={{ width:4, height:4, borderRadius:"50%", background:"currentColor" }} />
                          {r.available===true?"متوفر":r.available===false?"غير متوفر":"—"}
                        </span>
                        {r.link?<a href={r.link} target="_blank" rel="noopener noreferrer" style={{ fontSize:"12px", color:"#7c6af7", textDecoration:"none" }}>فتح ↗</a>:<span style={{color:"#aaaacc",fontSize:"12px"}}>—</span>}
                      </div>
                      {ai?.price_recommendation && ai.price_recommendation !== "unknown" && (
                        <div style={{ padding:"4px 16px 8px", display:"flex", alignItems:"center", gap:"6px" }}>
                          <span style={{ fontSize:"10px", color:"#666" }}>توصية:</span>
                          ...
                          <span style={{ fontSize:"10px", color:"#666" }}>{ai.price_recommendation_reason}</span>
                        </div>
                      )}
                    </div>
                  )})}
            ...
            <div>
              {!compExcelFile ? (
                <div onClick={()=>compExcelRef.current?.click()} style={{ border:"2px dashed #2a2a3e", borderRadius:"16px", padding:"56px", textAlign:"center", cursor:"pointer", background:"#f0f0f7" }}>
                  ...
                  <div style={{ fontSize:"14px", color:"#7c3aed", marginBottom:"4px" }}>اضغط لرفع ملف Excel</div>
                  <div style={{ fontSize:"12px", color:"#666" }}>يكتشف عمود SKU والاسم تلقائياً | أول 50 منتج</div>
                </div>
              ) : (
                <div>
                  <div style={{ background:"#f0f0f7", border:"1px solid #d8d8ee", borderRadius:"12px", padding:"12px 16px", display:"flex", alignItems:"center", gap:"10px", marginBottom:"14px", flexWrap:"wrap" }}>
                    ...
                    <span style={{ fontSize:"12px", color:"#666" }}>{compRows.length} منتج</span>
                    <div style={{ marginRight:"auto", display:"flex", gap:"8px", alignItems:"center" }}>
                      ...
                      <button onClick={()=>{setCompExcelFile("");setCompExcelRows([]);setCompRows([{id:"1",sku:"",query:"",status:"idle",results:[]}]);}} style={{background:"none",border:"none",color:"#666",cursor:"pointer",fontSize:"13px"}}>✕</button>
                    </div>
                  </div>
                  <div style={{ background:"#f0f0f7", border:"1px solid #d8d8ee", borderRadius:"10px", overflow:"hidden", marginBottom:"14px" }}>
                    <div style={{ padding:"9px 14px", borderBottom:"1px solid #d8d8ee", fontSize:"11px", color:"#666", fontWeight:"600" }}>معاينة</div>
                    <table style={{ width:"100%", borderCollapse:"collapse", fontSize:"12px" }}>
                      <thead><tr style={{background:"#ffffff"}}>
                        <th style={{padding:"7px 12px",textAlign:"right",color:"#666",borderBottom:"1px solid #d8d8ee",fontWeight:"500"}}>SKU</th>
                        <th style={{padding:"7px 12px",textAlign:"right",color:"#666",borderBottom:"1px solid #d8d8ee",fontWeight:"500"}}>اسم المنتج</th>
                      </tr></thead>
                      <tbody>
                        ...
                        {compRows.length>4&&<tr><td colSpan={2} style={{padding:"7px 12px",color:"#666",fontStyle:"italic"}}>... و {compRows.length-4} منتج آخر</td></tr>}
                      </tbody>
                    </table>
                  </div>
                  <div style={{ background:"#f0f0f7", border:"1px solid #d8d8ee", borderRadius:"10px", padding:"12px 14px", marginBottom:"14px" }}>
                    ...
                    <p style={{ fontSize:"11px", color:"#666", margin:"6px 0 0" }}>اتركه فارغاً للبحث في Homecenter وNoon تلقائياً</p>
                  </div>
                  <div style={{ display:"flex", gap:"8px" }}>
                    <button onClick={compSearchAll} disabled={compSearching}
                      style={{ padding:"11px 26px", background:compSearching?"#c8c8e8":"#7c3aed", color:compSearching?"#666":"#ffffff", border:"none", borderRadius:"10px", fontSize:"14px", fontWeight:"700", cursor:compSearching?"not-allowed":"pointer", fontFamily:"inherit" }}>
                      {compSearching?`جاري... (${compRows.filter(r=>r.status==="done").length}/${compRows.length})`:`ابدأ البحث في ${compRows.length} منتج ←`}
                    </button>
                    {compRows.some(r=>r.status==="done")&&(<button onClick={()=>setCompTab("results")} style={{padding:"11px 18px",background:"#f0f0f7",border:"1px solid #b8b8e0",borderRadius:"10px",color:"#7c3aed",fontSize:"13px",cursor:"pointer",fontFamily:"inherit"}}>النتائج →</button>)}
                  </div>
                </div>
              )}
            </div>
            ...
            <div>
              {!compRows.some(r=>r.status==="done") ? (
                <div style={{ background:"#f0f0f7", border:"1px dashed #2a2a3e", borderRadius:"14px", padding:"56px", textAlign:"center" }}>
                  ...
                  <p style={{color:"#666",fontSize:"13px"}}>لا توجد نتائج — ابدأ بحثاً أولاً</p>
                </div>
              ) : (
                <div>
                  <div style={{ display:"flex", alignItems:"center", gap:"10px", marginBottom:"14px", flexWrap:"wrap" }}>
                    ...
                    <button onClick={compExportExcel} style={{marginRight:"auto",padding:"7px 14px",background:"#e4e4f4",border:"1px solid #1e3a2e",borderRadius:"8px",color:"#80ffdb",fontSize:"12px",cursor:"pointer",fontFamily:"inherit"}}>تصدير Excel ↓</button>
                  </div>
                  <div style={{ background:"#f0f0f7", border:"1px solid #d8d8ee", borderRadius:"14px", overflow:"hidden" }}>
                    <div style={{overflowX:"auto"}}>
                      <table style={{width:"100%",borderCollapse:"collapse",fontSize:"12px",minWidth:"700px"}}>
                        <thead><tr style={{background:"#ffffff"}}>
                          {["SKU","المنتج","الموقع","الاسم عندهم","السعر","التوفر","رابط"].map(h=>(<th key={h} style={{padding:"9px 12px",textAlign:"right",color:"#666",fontWeight:"500",borderBottom:"1px solid #d8d8ee"}}>{h}</th>))}
                        </tr></thead>
                        <tbody>
                          {compRows.filter(r=>r.status==="done").flatMap(row=>
                            row.results.length===0
                              ?[<tr key={row.id+"-e"} style={{borderBottom:"1px solid #e0e0f0"}}><td style={{padding:"9px 12px",color:"#7c3aed",fontFamily:"monospace",fontSize:"11px"}}>{row.sku||"—"}</td><td style={{padding:"9px 12px",maxWidth:"150px",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{row.query}</td><td colSpan={5} style={{padding:"9px 12px",color:"#666",fontSize:"12px"}}>لا نتائج</td></tr>]
                              :row.results.map((r,i)=>(
                                <tr key={row.id+"-"+i} style={{borderBottom:"1px solid #e0e0f0",background:i%2?"#f2f2fa":"transparent"}}>
                                  ...
                                  <td style={{padding:"9px 12px"}}><span style={{fontSize:"11px",padding:"2px 8px",background:"#e4e4f4",color:"#7c3aed",borderRadius:"6px",fontWeight:"600"}}>{r.competitor}</span></td>
                                  ...
                                  <td style={{padding:"9px 12px"}}><span style={{display:"inline-flex",alignItems:"center",gap:"4px",padding:"2px 8px",borderRadius:"20px",fontSize:"11px",background:r.available===true?"#0d1f0d":r.available===false?"#fdeaea":"#e4e4f4",color:r.available===true?"#4ade80":r.available===false?"#f87171":"#666"}}><span style={{width:4,height:4,borderRadius:"50%",background:"currentColor"}}/>{r.available===true?"متوفر":r.available===false?"غير متوفر":"—"}</span></td>
                                  <td style={{padding:"9px 12px"}}>{r.link?<a href={r.link} target="_blank" rel="noopener noreferrer" style={{color:"#7c6af7",fontSize:"12px",textDecoration:"none"}}>فتح ↗</a>:<span style={{color:"#aaaacc"}}>—</span>}</td>
                                </tr>
                              ))
                          )}
                        </tbody>
    ...
    <div style={{ fontFamily: "'Tajawal', sans-serif", direction: "rtl", minHeight: "100vh", background: "#ffffff", color: "#1a1a2e" }}>
      ...
      <div style={{ display: "flex", minHeight: "100vh" }}>
        ...
        <div style={{ flex: 1, padding: "40px" }}>
          ...
          <p style={{ color: "#666", marginTop: "6px", fontSize: "14px", marginBottom: "32px" }}>هذه نظرة عامة على نشاطك</p>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: "16px", marginBottom: "32px" }}>
            {[{ label: "العمليات النشطة", value: "0", color: "#7c3aed" }, { label: "التكاملات", value: "0", color: "#80ffdb" }, { label: "المهام المكتملة", value: "0", color: "#ffd166" }, { label: "التوفير في الوقت", value: "0h", color: "#ff6b6b" }].map(stat => (
              <div key={stat.label} style={{ background: "#f0f0f7", border: "1px solid #d8d8ee", borderRadius: "16px", padding: "24px" }}>
                ...
                <div style={{ fontSize: "12px", color: "#666", marginTop: "6px" }}>{stat.label}</div>
              </div>
            ))}
          </div>
          <div style={{ background: "linear-gradient(135deg, #1a1a2e, #16213e)", border: "1px solid #b8b8e0", borderRadius: "20px", padding: "40px", textAlign: "center" }}>
            ...
            <div style={{ display: "flex", gap: "12px", justifyContent: "center" }}>
              ...
              <button onClick={() => setView("inventory")} style={{ background: "#e4e4f4", color: "#7c3aed", border: "1px solid #b8b8e0", borderRadius: "12px", padding: "12px 24px", fontSize: "14px", fontWeight: "700", cursor: "pointer" }}>مراقبة المخزون 📦</button>
            </div>
          </div>
        </div>
      </div>
    ...
    <div style={{ fontFamily: "'Tajawal', sans-serif", direction: "rtl", minHeight: "100vh", background: "#ffffff", display: "flex", alignItems: "center", justifyContent: "center" }}>
      ...
      <div style={{ width: "400px" }}>
        <div style={{ textAlign: "center", marginBottom: "40px" }}>
          ...
          <p style={{ color: "#666", marginTop: "8px", fontSize: "14px" }}>{view === "login" ? "سجل دخولك للمتابعة" : "أنشئ حساباً جديداً"}</p>
        </div>
        ...
        <div style={{ textAlign: "center", marginTop: "24px" }}>
          <span style={{ color: "#666", fontSize: "13px", cursor: "pointer" }} onClick={() => setView("landing")}>← العودة للرئيسية</span>
        </div>
      </div>
    </div>
  );
    ...
    <div style={{ fontFamily: "'Tajawal', sans-serif", direction: "rtl", background: "#ffffff", color: "#1a1a2e", minHeight: "100vh" }}>
      ...
      <nav style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "20px 60px", borderBottom: "1px solid #d8d8ee" }}>
        ...
        <div style={{ display: "flex", gap: "12px" }}>
          <button onClick={() => setView("login")} style={{ padding: "10px 24px", background: "transparent", border: "1px solid #b8b8e0", borderRadius: "10px", color: "#666", fontSize: "14px", cursor: "pointer" }}>دخول</button>
      ...
      <div style={{ textAlign: "center", padding: "100px 60px 80px" }}>
        ...
        <p style={{ fontSize: "18px", color: "#666", maxWidth: "500px", margin: "0 auto 40px", lineHeight: "1.8" }}>وصال يربط متاجرك، يدير طلباتك، ويشغّل AI agent يتصفح ويشتري بشكل تلقائي</p>
        <div style={{ display: "flex", gap: "12px", justifyContent: "center" }}>
          ...
          <button onClick={() => setView("login")} style={{ padding: "16px 36px", background: "transparent", border: "1px solid #b8b8e0", borderRadius: "14px", color: "#666", fontSize: "16px", cursor: "pointer" }}>تسجيل الدخول</button>
        </div>
      </div>
      <div style={{ textAlign: "center", padding: "24px", borderTop: "1px solid #d8d8ee", color: "#aaaacc", fontSize: "12px" }}>© 2025 وصال — جميع الحقوق محفوظة</div>
    </div>
  );
}
...
function ContentStudioView({ sidebarJSX, ENV_STYLES, CATEGORIES, SYSTEM_PROMPT, API_URL }: any) {
  ...
  const inStyle: React.CSSProperties = { width:"100%", padding:"8px 12px", background:"#f4f4fb", border:"1px solid #d0d0ec", borderRadius:"8px", color:"#4a4a7a", fontSize:"12px", outline:"none", fontFamily:"inherit", boxSizing:"border-box" };
  const tabs = [{k:"white",l:"خلفية بيضاء",i:"⬜"},{k:"env",l:"بيئة واقعية",i:"🏠"},{k:"dim",l:"مقاسات",i:"📐"},{k:"vars",l:"5 بيئات",i:"🎨"}];

  return (
    <div style={{ fontFamily:"'Tajawal',sans-serif", direction:"rtl", minHeight:"100vh", background:"#f8f8fc", color:"#1e1e3a" }}>
      <link href="https://fonts.googleapis.com/css2?family=Tajawal:wght@300;400;500;700;900&display=swap" rel="stylesheet" />
      <style>{"@keyframes spin{to{transform:rotate(360deg)}}"}</style>
      <div style={{ display:"flex", minHeight:"100vh" }}>
        {sidebarJSX}
        <div style={{ flex:1, display:"grid", gridTemplateColumns:"280px 1fr", overflow:"hidden" }}>
          ...
          {/* Left panel */}
          <div style={{ background:"#f2f2fa", borderLeft:"1px solid #d4d4f0", padding:"20px", overflowY:"auto", display:"flex", flexDirection:"column", gap:"14px" }}>
            <div>
              <h2 style={{ fontSize:"16px", fontWeight:"800", margin:"0 0 4px", color:"#fff" }}>✨ Content Studio AI</h2>
              <p style={{ fontSize:"12px", color:"#7070b0", margin:0 }}>توليد prompts احترافية لصور المنتجات</p>
            </div>
            ...
            <div>
              ...
              {imagePreview ? (
                <div style={{ position:"relative" }}>
                  <img src={imagePreview} alt="" style={{ width:"100%", height:"140px", objectFit:"contain", background:"#ebebf7", borderRadius:"10px", border:"1px solid #d0d0ec" }} />
                  <button onClick={() => { setImagePreview(null); setImageBase64(null); }} style={{ position:"absolute", top:6, left:6, width:22, height:22, background:"#fdeaea", border:"1px solid #3a1a1a", borderRadius:"50%", color:"#f87171", cursor:"pointer", fontSize:"11px" }}>✕</button>
                </div>
              ) : (
                <div onClick={() => fileRef.current?.click()} style={{ border:"2px dashed #1e1e3a", borderRadius:"10px", padding:"24px", textAlign:"center", cursor:"pointer", background:"#f4f4fb" }}>
                  <input ref={fileRef} type="file" accept="image/*" onChange={handleImageUpload} style={{ display:"none" }} />
                  <div style={{ fontSize:"24px", marginBottom:"6px" }}>📷</div>
                  <div style={{ fontSize:"12px", color:"#8080c0" }}>ارفع صورة المنتج</div>
                </div>
              )}
            </div>
            ...
            <div>
              ...
              <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr 1fr", gap:"6px" }}>
                {([["العرض",dimW,setDimW],["العمق",dimD,setDimD],["الارتفاع",dimH,setDimH]] as [string,string,any][]).map(([l,v,s]) => (
                  <div key={l}>
                    <div style={{ fontSize:"10px", color:"#8080c0", marginBottom:"3px" }}>{l}</div>
            ...
            <div>
              ...
              <div style={{ display:"flex", gap:"6px" }}>
                {["Arabic","English"].map(l => (
                  <button key={l} onClick={() => setLabelLang(l)} style={{ flex:1, padding:"7px", background:labelLang===l?"#1e1a3a":"#f4f4fb", border:`1px solid ${labelLang===l?"#7c3aed":"#d8d8f0"}`, borderRadius:"7px", color:labelLang===l?"#6d28d9":"#8080c0", fontSize:"12px", cursor:"pointer", fontFamily:"inherit" }}>{l}</button>
                ))}
              </div>
            </div>
            ...
            <button onClick={handleGenerate} disabled={loading || (!imageBase64 && !imageUrl && !category)}
              style={{ width:"100%", padding:"13px", background:loading?"#dcdcf4":"linear-gradient(135deg,#7c3aed,#db2777)", border:"none", borderRadius:"10px", color:loading?"#7070b0":"#fff", fontSize:"14px", fontWeight:"800", cursor:loading?"not-allowed":"pointer", fontFamily:"inherit" }}>
              {loading ? "جاري التوليد..." : "✨ توليد الـ Prompts"}
            </button>
          </div>

          {/* Right: Results */}
          <div style={{ padding:"24px", overflowY:"auto", background:"#f8f8fc" }}>
            {loading && (
              <div style={{ textAlign:"center", padding:"60px 0" }}>
                ...
                <p style={{ color:"#7070b0", fontSize:"13px" }}>Claude يولد الـ prompts...</p>
              </div>
            )}

            {error && <div style={{ padding:"12px 16px", background:"#fdeaea", border:"1px solid #3a1a1a", borderRadius:"10px", color:"#f87171", fontSize:"13px", marginBottom:"16px" }}>⚠ {error}</div>}

            {result && !loading && (
              <>
                {result.product_detected && (
                  <div style={{ background:"#edfaed", border:"1px solid #1a3a1a", borderRadius:"10px", padding:"10px 14px", marginBottom:"16px" }}>
                    <span style={{ fontSize:"12px", color:"#4ade80" }}>✅ تم اكتشاف: {result.product_detected}</span>
                  </div>
                )}
                <div style={{ display:"flex", borderBottom:"1px solid #d4d4f0", marginBottom:"16px" }}>
                  {tabs.map(t => (
                    <button key={t.k} onClick={() => setActiveTab(t.k)}
                      style={{ padding:"8px 14px", background:"none", border:"none", borderBottom:activeTab===t.k?"2px solid #7c3aed":"2px solid transparent", color:activeTab===t.k?"#6d28d9":"#7070b0", fontSize:"12px", fontWeight:activeTab===t.k?"700":"400", cursor:"pointer", fontFamily:"inherit", marginBottom:"-1px" }}>
                      {t.i} {t.l}
                    </button>
                  ))}
                </div>
                ...
                {result.notes && (
                  <div style={{ background:"#f4f4fb", border:"1px solid #1e2a3a", borderRadius:"8px", padding:"10px 14px", marginTop:"8px" }}>
            ...
            {!result && !loading && !error && (
              <div style={{ textAlign:"center", padding:"80px 0" }}>
                ...
                <p style={{ color:"#8080c0", fontSize:"14px", marginBottom:"6px" }}>ارفع صورة المنتج وحدد الإعدادات</p>
                <p style={{ color:"#a0a0d8", fontSize:"12px" }}>سيولد Claude prompts احترافية جاهزة لأدوات توليد الصور</p>
              </div>
            )}
          </div>
        </div>
...
function PromptBox({ label, prompt, id, copied, onCopy, onGenerate, genLoading, genImage }: {
  ...
  return (
    <div style={{ background:"#f4f4fb", border:"1px solid #d0d0ec", borderRadius:"12px", padding:"16px", marginBottom:"12px" }}>
        ...
        <div style={{ display:"flex", gap:"6px" }}>
          {onGenerate && (
            <button onClick={() => onGenerate(prompt, id)} disabled={isGenerating}
              style={{ padding:"4px 12px", background:isGenerating?"#e4e4f4":"linear-gradient(135deg,#7c3aed,#db2777)", border:"none", borderRadius:"6px", color:isGenerating?"#666":"#fff", fontSize:"11px", cursor:isGenerating?"not-allowed":"pointer", fontFamily:"inherit", whiteSpace:"nowrap" }}>
              {isGenerating ? "جاري التوليد..." : "🎨 ولّد الصورة"}
            </button>
          )}
          <button onClick={() => onCopy(prompt, id)}
            style={{ padding:"4px 12px", background:copied===id?"#d0f0d0":"#dcdcf4", border:`1px solid ${copied===id?"#4ade80":"#b0b0e0"}`, borderRadius:"6px", color:copied===id?"#4ade80":"#6060c0", fontSize:"11px", cursor:"pointer", fontFamily:"inherit" }}>
            {copied===id ? "✓ تم النسخ" : "نسخ"}
          </button>
        </div>
      </div>
      <p style={{ fontSize:"12px", color:"#5a5a8a", lineHeight:"1.7", margin:"0 0 12px", direction:"ltr", textAlign:"left", whiteSpace:"pre-wrap" }}>{prompt}</p>
      {genImage && (
        <div style={{ borderTop:"1px solid #d0d0ec", paddingTop:"12px" }}>
          <img src={genImage} alt="Generated" style={{ width:"100%", borderRadius:"8px", border:"1px solid #b8b8e0" }} />
          <a href={genImage} download={`${id}_generated.jpg`} target="_blank" rel="noopener noreferrer"
            style={{ display:"inline-block", marginTop:"8px", padding:"5px 14px", background:"#dcdcf4", border:"1px solid #2a2a5a", borderRadius:"6px", color:"#6d28d9", fontSize:"11px", textDecoration:"none" }}>
            ⬇ تنزيل الصورة
          </a>
        </div>
      )}
