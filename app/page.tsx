...
export default function Home() {
  ...
  if (view === "reports") {
    ...
    const renderDetail = (idx: number) => {
      ...
      if (key === "late_orders" || key === "stuck_orders") {
        ...
        return (
          <table style={{ width:"100%", borderCollapse:"collapse", fontSize:"12px" }}>
            <thead><tr style={{ borderBottom:"1px solid #1e1e2e" }}>{["الطلب","العميل", key === "late_orders" ? "أيام التأخير" : "الحالة","التاريخ","المبلغ"].map(h=><th key={h} style={{ padding:"6px 8px", textAlign:"right", color:"#555", fontWeight:"500" }}>{h}</th>)}</tr></thead>
            <tbody>{items.map((o: any) => (
              <tr key={o.id} style={{ borderBottom:"1px solid #141420" }}>
                <td style={{ padding:"6px 8px", color:"#c8b8ff", fontSize:"11px" }}>{o.id}</td>
                <td style={{ padding:"6px 8px" }}>{o.customer}</td>
                <td style={{ padding:"6px 8px" }}><span style={{ background:"#1f0d0d", color:"#f87171", padding:"1px 7px", borderRadius:"10px", fontSize:"10px" }}>{key === "late_orders" ? `${o.days_late} يوم` : o.status}</span></td>
                <td style={{ padding:"6px 8px", color:"#888", fontSize:"11px" }}>{o.delivery_date || o.date}</td>
                <td style={{ padding:"6px 8px", color:"#fbbf24", fontSize:"11px" }}>{o.amount ? `${Number(o.amount).toLocaleString()} ر.س` : "—"}</td>
              </tr>
            ))}</tbody>
          </table>
        );
      ...
      if (key === "out_of_stock" || key === "low_stock") {
        ...
        return (
          <table style={{ width:"100%", borderCollapse:"collapse", fontSize:"12px" }}>
            <thead><tr style={{ borderBottom:"1px solid #1e1e2e" }}>{["SKU","المستودع","الكمية"].map(h=><th key={h} style={{ padding:"6px 8px", textAlign:"right", color:"#555", fontWeight:"500" }}>{h}</th>)}</tr></thead>
            <tbody>{items.map((i: any, idx: number) => (
              <tr key={idx} style={{ borderBottom:"1px solid #141420" }}>
                <td style={{ padding:"6px 8px", color:"#c8b8ff" }}>{i.sku}</td>
                <td style={{ padding:"6px 8px", color:"#888" }}>{i.warehouse}</td>
                <td style={{ padding:"6px 8px" }}><span style={{ background: key === "out_of_stock" ? "#1f0d0d" : "#1a1400", color: key === "out_of_stock" ? "#f87171" : "#fbbf24", padding:"1px 7px", borderRadius:"10px", fontSize:"10px" }}>{i.qty}</span></td>
              </tr>
            ))}</tbody>
          </table>
        );
      ...
      if (key === "late_po") {
        ...
        return (
          <table style={{ width:"100%", borderCollapse:"collapse", fontSize:"12px" }}>
            <thead><tr style={{ borderBottom:"1px solid #1e1e2e" }}>{["PO","المورد","الاستحقاق","الحالة"].map(h=><th key={h} style={{ padding:"6px 8px", textAlign:"right", color:"#555", fontWeight:"500" }}>{h}</th>)}</tr></thead>
            <tbody>{items.map((o: any) => (
              <tr key={o.id} style={{ borderBottom:"1px solid #141420" }}>
                <td style={{ padding:"6px 8px", color:"#c8b8ff", fontSize:"11px" }}>{o.id}</td>
                <td style={{ padding:"6px 8px" }}>{o.supplier}</td>
                <td style={{ padding:"6px 8px", color:"#f87171" }}>{o.due}</td>
                <td style={{ padding:"6px 8px" }}><span style={{ background:"#1f0d0d", color:"#f87171", padding:"1px 7px", borderRadius:"10px", fontSize:"10px" }}>{o.status}</span></td>
              </tr>
            ))}</tbody>
          </table>
        );
      ...
      if (key === "open_complaints") {
        ...
        return (
          <table style={{ width:"100%", borderCollapse:"collapse", fontSize:"12px" }}>
            <thead><tr style={{ borderBottom:"1px solid #1e1e2e" }}>{["#","العميل","الموضوع","الحالة"].map(h=><th key={h} style={{ padding:"6px 8px", textAlign:"right", color:"#555", fontWeight:"500" }}>{h}</th>)}</tr></thead>
            <tbody>{items.map((i: any) => (
              <tr key={i.id} style={{ borderBottom:"1px solid #141420" }}>
                <td style={{ padding:"6px 8px", color:"#c8b8ff", fontSize:"11px" }}>{i.id}</td>
                <td style={{ padding:"6px 8px" }}>{i.customer}</td>
                <td style={{ padding:"6px 8px", color:"#888" }}>{i.subject}</td>
                <td style={{ padding:"6px 8px" }}><span style={{ background:"#1a1400", color:"#fbbf24", padding:"1px 7px", borderRadius:"10px", fontSize:"10px" }}>{i.status}</span></td>
              </tr>
            ))}</tbody>
          </table>
        );
      }
      // Metric cards for the rest
      ...
      return (
        <div style={{ display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap:"8px" }}>
          {cards.map(([l,v,s,c]) => (
            <div key={l} style={{ background:"#0f0f1a", borderRadius:"8px", padding:"12px", textAlign:"center" }}>
              <div style={{ fontSize:"10px", color:"#666", marginBottom:"4px" }}>{l}</div>
              <div style={{ fontSize:"20px", fontWeight:"600", color:c, margin:"4px 0" }}>{v}</div>
      ...
      <div style={{ fontFamily:"'Tajawal', sans-serif", direction:"rtl", minHeight:"100vh", background:"#0a0a0f", color:"#e8e8f0" }}>
        ...
        <div style={{ display:"flex", minHeight:"100vh" }}>
          ...
          <div style={{ flex:1, padding:"32px", overflowY:"auto" }}>
              ...
              <div>
                ...
                <div style={{ display:"flex", alignItems:"center", gap:"8px" }}>
                  {erpLoading
                    ...
                    : erpError
                      ? <span style={{ fontSize:"12px", color:"#f87171" }}>⚠️ {erpError}</span>
                      : <span style={{ fontSize:"12px", color:"#4ade80" }}>● ERPNext متصل {lastFetched && `· آخر تحديث: ${lastFetched}`}</span>
                  }
                </div>
              </div>
            ...
            {/* Error Banner */}
            {erpError && (
              <div style={{ background:"#1a0a0a", border:"1px solid #3a1a1a", borderRadius:"10px", padding:"12px 16px", marginBottom:"16px", display:"flex", alignItems:"center", gap:"10px" }}>
                <span style={{ color:"#f87171", fontSize:"13px" }}>⚠️ تعذّر الاتصال بـ ERPNext</span>
                <button onClick={fetchKpis} style={{ marginRight:"auto", padding:"4px 12px", background:"#2a1a1a", border:"1px solid #5a2a2a", borderRadius:"6px", color:"#f87171", fontSize:"11px", cursor:"pointer", fontFamily:"inherit" }}>إعادة المحاولة</button>
              </div>
            )}
            ...
            <div style={{ display:"grid", gridTemplateColumns:"repeat(5,1fr)", gap:"10px", marginBottom:"10px" }}>
              {kpis.slice(0,5).map((k,i)=>(
                <div key={i} onClick={()=>setActiveKpi(activeKpi===i?null:i)}
                  style={{ background:"#111118", border:`1px solid ${activeKpi===i?"#378ADD":"#1e1e2e"}`, borderTop:`3px solid ${k.color}`, borderRadius:"12px", padding:"14px", cursor:"pointer", position:"relative", boxShadow:activeKpi===i?"0 0 0 2px rgba(55,138,221,0.2)":"none", transition:"all .15s" }}>
                  ...
                  <div style={{ fontSize:"22px", fontWeight:"700", color:k.color, marginBottom:"3px" }}>
            ...
            <div style={{ background:"#111118", border:"1px solid #1e1e2e", borderRadius:"14px", overflow:"hidden" }}>
              <div style={{ padding:"14px 20px", borderBottom:"1px solid #1e1e2e", fontSize:"13px", fontWeight:"600", display:"flex", alignItems:"center", justifyContent:"space-between" }}>
                <span>ملخص تنفيذي — حالة جميع KPIs</span>
                {d && <span style={{ fontSize:"11px", color:"#555" }}>مصدر: ERPNext · {lastFetched}</span>}
              </div>
              <table style={{ width:"100%", borderCollapse:"collapse", fontSize:"12px" }}>
                ...
                <tbody>
                  {kpis.map((k, i) => (
                    <tr key={i} style={{ borderBottom:"1px solid #141420", cursor:"pointer" }} onClick={()=>setActiveKpi(activeKpi===i?null:i)}>
                      ...
                      <td style={{ padding:"8px 12px" }}>{k.n.replace(/^[①-⑩] /, "")}</td>
                      <td style={{ padding:"8px 12px", fontWeight:"600", color:k.color }}>{k.val}</td>
                      <td style={{ padding:"8px 12px", color:"#888" }}>{k.target}</td>
                      <td style={{ padding:"8px 12px", color:k.color }}>{k.trend || "—"}</td>
                      <td style={{ padding:"8px 12px" }}>
                        <span style={{ background:k.color==="#1D9E75"?"#0d1f0d":k.color==="#E24B4A"?"#1f0d0d":"#1a1400", color:k.color, padding:"2px 8px", borderRadius:"10px", fontSize:"10px" }}>
                          {k.color==="#1D9E75" ? "✓ جيد" : k.color==="#E24B4A" ? "⚠ حرج" : "~ تحذير"}
                        </span>
                      </td>
                    </tr>
                  ))}
